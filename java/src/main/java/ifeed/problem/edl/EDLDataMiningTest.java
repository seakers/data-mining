...
public class EDLDataMiningTest {
    ...
    public static void main(String[] args) {
        ...
        double pmin = 0.05;
        double[] epsilonDouble = new double[]{0.03, 0.03, 1};
        ...
        String inputDataFile = "/Users/bang/workspace/edl_data_mining/beam1fail_mix.dat";

        InputDatasetReader reader = new InputDatasetReader(inputDataFile);
        reader.setHeaader(0);
        reader.setInputType(InputDatasetReader.InputType.DISCRETE);
        reader.setColumnInfo(InputDatasetReader.ColumnType.CLASSLABEL,1);
        for(int i = 2; i < 20; i++){
            reader.setColumnInfo(InputDatasetReader.ColumnType.DECISION, i);
        }
        reader.readData();
        ...
        List<String> decisionVarNames = reader.getDecisionVarNames();
        List<String> objectiveVarNames = reader.getObjectiveVarNames();
        String labelVarName = reader.getLabelName();

        // Set params obejct
        Params params = new Params(decisionVarNames.size());
        params.setDecisionVarNames(decisionVarNames);
        params.setObjectiveVarNames(objectiveVarNames);
        params.setLabelVarName(labelVarName);
        params.computeMaxValues(architectures);
        ...
        switch (mode) {
            case MOEA_RULESET:
                ...
                for (int i = 0; i < numRuns; i++) {
                    ...
                    System.out.println("base features: "+ base.getBaseFeatures().size());
