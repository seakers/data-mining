/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package ifeed_dm.EOSS.filtersWithRepairOperator;

import ifeed_dm.EOSS.EOSSParams;

import java.util.Random;
import java.util.BitSet;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;

/**
 *
 * @author bang
 */
public class Separate extends ifeed_dm.EOSS.filters.Separate implements RepairOperators {

    public Separate(int[] instruments){
        super(instruments);
    }

    @Override
    public BitSet disrupt(BitSet input){

        if(!super.apply(input)){
            // Do nothing
            return input;
        }else{
            // Satisfies all constraints
            Random random = new Random();
            // Select orbit
            int max = 4;
            int min = 0;
            int randOrb = random.nextInt(max + 1 - min) + min;

            // Select two instruments
            int instr1;
            int instr2;
            if(super.instruments.length == 2){
                instr1 = super.instruments[0];
                instr2 = super.instruments[1];
            }else{
                ArrayList<Integer> list = new ArrayList<>();
                for(int i:super.instruments){
                    list.add(i);
                }
                Collections.shuffle(list);

                instr1 = list.get(0);
                instr2 = list.get(1);
            }

            BitSet out = (BitSet) input.clone();
            out.set(randOrb * EOSSParams.num_instruments + instr1);
            out.set(randOrb * EOSSParams.num_instruments + instr2);
            return out;
        }
    }

    @Override
    public BitSet repair(BitSet input){
        if(super.apply(input)){
            // Do nothing
            return input;
        }else{

            Random random = new Random();
            ArrayList<Integer> orbitsWithTargetInstruments = new ArrayList<>();

            for(int o=0;o<EOSSParams.num_orbits;o++){
                boolean together = true;
                for(int i:instruments){
                    if(!input.get(o * EOSSParams.num_instruments + i)){
                        // If any one of the instruments are not present
                        together=false;
                        break;
                    }
                }

                if(together){
                    orbitsWithTargetInstruments.add(o);
                    break;
                }
            }

            BitSet out = (BitSet) input.clone();

            for(int o:orbitsWithTargetInstruments){
                ArrayList<Integer> usedInstruments = new ArrayList<>();
                for(int i:instruments){
                    if(input.get(o * EOSSParams.num_instruments + i)){
                        usedInstruments.add(i);
                    }
                }

                // Select only one instrument and remove all the rest
                Collections.shuffle(usedInstruments);
                usedInstruments.remove(0);
                for(int i:usedInstruments){
                    out.clear(o * EOSSParams.num_instruments + i);
                }

            }

            return out;

        }
    }
}
